...
def test_un_no_es_una_respuesta_que_se_guarda():
    ...
    r = Respuesta(pregunta="PREPAGADA", tiene=False, detalle={}, quien="cliente",
                  cuando=AHORA)
...
def test_el_detalle_viaja_con_la_respuesta():
    r = Respuesta(pregunta="DEPENDIENTES", tiene=True,
                  detalle={"cuantos": 2, "tipo": "hijo_menor"},
                  quien="cliente", cuando=AHORA)
...
def test_una_clave_desconocida_revienta():
    ...
    with pytest.raises(ValidationError):
        Respuesta(pregunta="PREPAGADA", tiene=False, detalle={}, quien="cliente",
                  cuando=AHORA, respuesta="si")
