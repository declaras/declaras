...
def validar(caso: CasoTributario, p: ParametrosAnio, t: Traza) -> None:
    ...
    ingresos = (
        [(f"laboral {l.empleador_nombre}", l.fuente, l.retencion, l.bruto) for l in caso.laborales]
        + [
            (f"pensión {pn.pagador}", pn.fuente, pn.retencion, sum(pn.mesadas))
            for pn in caso.pensiones
        ]
        + [(f"rendimientos {r.entidad}", r.fuente, r.retencion, r.valor) for r in caso.rendimientos]
        + [(f"arriendo {a.inmueble}", a.fuente, a.retencion, a.canon_total) for a in caso.arriendos]
        + [
            (f"dividendos {d.sociedad_nombre}", d.fuente, d.retencion, d.no_gravados + d.gravados)
            for d in caso.dividendos
        ]
    )
